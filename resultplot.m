...
figure1 = figure('InvertHardcopy','off','PaperType','A4',...
...
for iSyn=1:SynNum
    ...
    ParLab=sprintf("%s_100",Ws.ParLabels{iSyn});
    M.wc=wc;
    xFig=Ws.(ParLab).ParValues;
%     M.wc=Ws.(ParLab).ParValues;
...
%         ylim([y1min-scaledif*0.2 y1max+scaledif*0.2]);
        ...
        for FilterNum=1:3
            plot(xFig(BaseLineVal),FittedVal(FilterNum,BaseLineVal),'*','Color','k')
        end
        
%         if iParam==4
%             xlabel({xAxisLabels{1,iSyn}; xAxisLabels{2,iSyn} } )
%         end
...
orient(figure1,'landscape')
print(figure1,'syn.pdf','-dpdf')
...
for iPar=1:5
    ...
    Ws.(ParLabel).ParValues=ParValues;
    Ws.ParLabels=VarLabels;
    for iFilt=1:3
...
%         subplot(5,5,15+iPar)
        ...
        disty=1-2*ymargin;
        mrg=0.015;
        PlotLengthX=disty/(ParamNum);
        PlotLengthY=distx/SynNum;
