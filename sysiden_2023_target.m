...
for iFilter=1:FilterNum
    ...
    for iVal1=1:Val1Num
        ...
        %             [FreqRespWelch, f_welch]=tfestimate(x_v,y_v,200,[100],[f_onesided],fs/DownSampleRate);
                ...
                O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).PhaseWelchOneSided=angle(FT_L(1:NumUniquePts));
                O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).MagWelchOneSided=abs(FT_L(1:NumUniquePts));
...
%             AvgPhase=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).AvgPhase(1,:);
            ...
            FreqRespWelchOneSided=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).FreqRespWelchOneSided(2:end);
            FreqRespOneSided=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).FreqRespOneSided(2:end);
    ...
    %             MagAllRepsOneSided=O.(ParLabel).(FiltLabel).(ValLabel).MagAllRepsOneSided(1,:);
            ...
            figure(iVal1)
            subplot(3,1,iFilt)
            semilogx(f_onesided,20*log10((FreqRespWelchOneSided)),'DisplayName','Welch')
            hold on
    ...
    for iVal1=1:Val1Num
        ...
        for iVal2=1:Val2Num
            ...
            for iRep=1:1 % fitting for all the repeats 
                ...
                [FittedFreqResp SysTf x]= sysiden_basic2(Mag,x0,lb,ub,FreqVec,Estnp,Estnz,Ts);
                ...
                O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).x=x;

                O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).FittedFreqResp=FittedFreqResp';
                
                MagWelch=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).FreqRespWelchOneSided(FreqIndice);
                FreqRespWelchOneSided=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).FreqRespWelchOneSided(FreqIndice);
                FreqResp=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).FreqRespWelchOneSided(FreqIndice)';
...
% 
    ...
    %             x0=Detx0(Estnp+Estnz+1,lb,ub);
                [FittedFreqResp SysTf]= sysiden_basic2(Mag,x0,[],[],FreqVec,Estnp,Estnz,Ts);
                SysFreq = frd(FreqResp,FreqVec,'FrequencyUnit','Hz','Ts',Ts);
...
%             plotrange=1:NumUniquePts;
            figure(iVal2)
            subplot(2,1,1)
            semilogx(f_onesided,20*log10((MagL(1:end))))
            hold on
...
%             semilogx(f_onesided,zeros(1,length(f_onesided)),'k')
            ...
            legend(ConfIntLabel)
%             subplot(2,1,2)
%             semilogx(FreqVec,20*log10(abs(MagWelch)))
%             hold on
...
%                 
%                 WelchFittedFreqResp=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).WelchFittedFreqResp;
%                 MagWelchAllRepsOneSided=O.(FiltLabel).(Val1Label).(Val2Label).(RepeatLabel).MagWelchOneSided(FreqIndice);
% 
%                 figure(12)
%                 ColorLabel=lines(10);
%                 subplot(2,1,1)
...
function [Wtest Wtf x]=sysiden_basic2(FreqResp,x0,lb,ub,FreqVector,np,nz,Ts)
% system indentification using fmincon\
global minfThres IterLim FreqVec
minf=100000;
nIter=0;
