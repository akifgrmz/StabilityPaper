...
plot(mean(xs), y*1.18, '*k')% the sig star sign
if lbl
    text(mean(xs)*1.1, y*1.18, lbl)% the sig star sign
end
plot([1;1]*xs(1),[y*1.07,y*1.1],'-k', 'LineWidth',2);%left edge drop
plot([1;1]*xs(2),[y*1.07,y*1.1],'-k', 'LineWidth',2);%right edge drop
hold off
