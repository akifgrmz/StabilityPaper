...
mdlWks = get_param('SysIden_2023','ModelWorkspace');
...
AdressString{1}='SysIden_2023/GainP';
BlockString{1}='Gain';
NominalString{1}='2';
VarLabels{1}='Phand';
...
AdressString{2}='SysIden_2023/Paretic Hand/Stim Hand Angle/Transfer Fcn';
...
AdressString{3}='SysIden_2023/Paretic Hand/Occlusion/Noise';
BlockString{3}='Value';
NominalString{3}='20';
VarLabels{3}='OccK';
...
AdressString{4}='SysIden_2023/Paretic Hand/mWave/GainVar';
BlockString{4}='Gain';
...
AdressString{5}='SysIden_2023/Paretic Hand/mWave/GainVar';
BlockString{5}='Gain';
NominalString{5}='20';
...
low=0;
high=3;
TargetVar=logspace(log10(low),log10(high),NumPoints);
AdressString{6}='SysIden_2023/Subsystem1/Mean';
BlockString{6}='Value';
NominalString{6}='0';
VarLabels{6}='TargetLevel';
O.(VarLabels{6}).ParValues=TargetVar;
O.(VarLabels{6}).AdressString=AdressString{6};
O.(VarLabels{6}).NomValue=0;

for i=1:length(TargetVar)
    TargetVarString{i}=sprintf('%0.8f',TargetVar(i));
end
...
for iFilter=1:NumFilter
    ...
    set_param('SysIden_2023/Effort Estimator/FilterNum','Value',str1)
